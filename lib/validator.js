...
function formatValidationErrors(errors, configuration) {
  ...
  const result = errorsWithPriority
    .sort(sortValidationErrors)
    .map(err => err.message)
    .join(' and ');
...
function addPriorities(configuration) {
  return function (err) {
    const [section, position = 99, name = ''] = err.instancePath.replace(/^\//, '').split('/');
    const order = configuration?.order?.blocks || [];
...
function sortValidationErrors(a, b) {
  // prettier-ignore
